...
def get_category_event_candidates(category_path):
    ...
    Now properly finds all event links from the category page.
    """
    ...
    # Method 1: Find events from table rows (like in the homepage)
        ...
        for row in rows:
            ...
            if link_tag and link_tag.get("href"):
                ...
                if href and not href.startswith(("mailto:", "javascript:")):
                    ...
                    # Skip if it's a category link (like /soccer, /mlb, etc.)
                    if full in [urljoin(BASE_URL, cat) for cat in ["/soccer", "/mlb", "/nba", "/nfl", "/nhl", "/fighting", "/motorsports"]]:
                        continue
    ...
    # Method 2: Find all anchors with event-like patterns
    for a in soup.find_all("a", href=True):
        href = a["href"].strip()
        ...
        if (".m3u8" in href or 
            any(k in low for k in ("stream", "streams", "match", "game", "event", "ppv")) or 
            re.search(r"-\d+$", low) or
            re.search(r"streams-\d+", low)):
            if full not in seen:
                seen.add(full)
                clean_text = clean_event_title(text)
                if not clean_text or clean_text.startswith("http"):
    ...
    # Method 3: If no candidates found, look for any links on the page
    if not candidates:
        for a in soup.find_all("a", href=True):
            href = a["href"].strip()
            if href and not href.startswith(("#", "mailto:", "javascript:")):
                full = href if href.startswith("http") else urljoin(cat_url, href)
                if full not in seen and full != cat_url:
                    seen.add(full)
                    text = a.get_text(" ", strip=True)
                    clean_text = clean_event_title(text) if text else ""
                    candidates.append((clean_text, full))
    
    # If still nothing found, search inline for m3u8
    if not candidates:
