...
def filter_by_country(channel: dict) -> bool:
    """Filter channels by allowed country codes"""
    try:
        # Get country code from channel data
        country_code = channel.get("code", "").lower()
        
        # If no country code, try to extract from URL or other fields
        if not country_code:
        ...
        if country_code in ALLOWED_COUNTRIES:
            ...
            log.debug(f"Channel {channel.get('name')} from {country_name} (code: {country_code}) - INCLUDED")
            return True
        else:
            if country_code:
                log.debug(f"Channel {channel.get('name')} from country code {country_code} - EXCLUDED")
            else:
                log.debug(f"Channel {channel.get('name')} has no country code - EXCLUDED")
            return False
            
    except Exception as e:
        log.error(f"Error filtering channel {channel.get('name')}: {e}")
        return False
...
async def get_events(cached_keys: list[str]) -> list[dict[str, str]]:
            ...
            try:
                ...
                if api_data:
                    ...
                    for channel in api_data:
                        if filter_by_country(channel):
                            filtered_count += 1
                    log.info(f"After country filter: {filtered_count} channels from allowed countries")
                else:
    ...
    log.info(f"Processing channels from API with country filter")
    
    for channel in api_data:
        try:
            # Apply country filter
            if not filter_by_country(channel):
                continue
            ...
            # Get country code for group title
            country_code = channel.get("code", "").lower()
            country_name = ALLOWED_COUNTRIES.get(country_code, "Live Events")
            
            # Get league/sport from category or tournament, use country name if available
            league = channel.get("category", channel.get("tournament", country_name))
            ...
            # Get logo URL
            logo = channel.get("image", channel.get("logo", channel.get("icon", "")))
            
            # Get channel ID for tvg-id
            channel_id = channel.get("id", channel.get("channel_id", ""))
            if not channel_id:
                # Generate ID from name and country
                channel_id = f"{channel_name.replace(' ', '.')}.{country_code}".lower()
            ...
            log.info(f"Found new channel: {key} from {country_name}")
            
        except Exception as e:
            log.error(f"Error processing channel: {e}")
            continue
    
    log.info(f"Total new channels found after filtering: {len(events)}")
    return events
...
async def scrape(browser: Browser) -> None:
    ...
    log.info(f"Country filter enabled for: {', '.join(ALLOWED_COUNTRIES.values())}")
        ...
        async with network.event_context(browser) as context:
            for i, ev in enumerate(events, start=1):
                async with network.event_page(context) as page:
                    log.info(f"Processing event {i}/{len(events)}: {ev['sport']} - {ev['event']} ({ev.get('country_name', 'Unknown')})")
...
async def main():
    ...
    log.info("Starting CDNTV_CH updater with country filter")
    ...
    log.info(f"Allowed countries: {', '.join(ALLOWED_COUNTRIES.values())}")
